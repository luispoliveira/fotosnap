      ...
      <DialogContent className="max-w-md! w-full h-[90vh] p-0 overflow-hidden bg-black" showCloseButton={false}>
        <div className="relative w-full h-full flex items-center justify-center">
          ...
          <div className="relative w-full h-full">
            <Image src={getImageUrl(currentStory.image)}
              alt="Story" fill className="object-contain"
            />
          </div>

          {
            (currentGroupIndex > 0 || currentStoryIndex > 0) && (
              <Button variant={"ghost"} size={"icon"} onClick={handlePrevious} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 text-white bg-black/50 hover:bg-black/70 transition-colors">
                <ChevronLeft className="w-6 h-6" />
              </Button>
            )
          }
          <Button variant={"ghost"} size={"icon"} onClick={handleNext} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 text-white bg-black/50 hover:bg-black/70 transition-colors">
