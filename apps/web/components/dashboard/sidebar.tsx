        ...
        <div className="space-y-3">
          {
            mockSuggestions.map((user) => (
              <div key={user.id} className="flex items-center space-x-2">
                <Image src={user.avatar} alt={user.username} className="w-8 h-8 rounded-full" width={40} height={40} />
                <div className="flex-1 min-w-0">
                ...
                <Button variant={"ghost"} size={"sm"} className="text-primary hover:text-primary/90 text-xs">
                  Follow
                </Button>
              </div>
            ))
