...
export default function Sidebar() {
  ...
  const { data: suggestedUsers = [] } = trpc.usersRouter.getSuggestedUsers.useQuery();
            ...
            suggestedUsers.length === 0 ? (
              ...
              suggestedUsers.map((user) => (
                <div key={user.id} className="flex items-center justify-between">
                  <Button variant={"ghost"} onClick={() => router.push(`/users/${user.id}`)}
                  ...
                  <Button onClick={() => followMutation.mutate({ userId: user.id })} variant={"ghost"} size={"sm"} className="text-primary hover:text-primary/90 text-xs" disabled={followMutation.isPending}>
                    Follow
                  </Button>
                </div>
              ))
